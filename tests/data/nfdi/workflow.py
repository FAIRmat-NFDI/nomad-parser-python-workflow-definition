...
def convert_to_xdmf(gmsh_output_file: str) -> dict:
    ...
    _ = check_output(
        [
            'meshio',
            'convert',
            os.path.basename(gmsh_output_file),
            meshio_output_file_name,
        ],
        prefix_name=stage_name,
        cwd=stage_name,
        universal_newlines=True,
...
def poisson(
    meshio_output_xdmf: str, meshio_output_h5: str, source_directory: str
) -> dict:
    ...
    _ = check_output(
        [
            ...
            'poisson.py',
            '--mesh',
            os.path.basename(meshio_output_xdmf),
            '--degree',
            '2',
            '--outputfile',
            poisson_output_pvd_file_name,
            '--num-dofs',
            poisson_output_numdofs_file_name,
        ],
        prefix_name=stage_name,
        cwd=stage_name,
        universal_newlines=True,
    ...
    return {
        'numdofs': _poisson_collect_output(
            numdofs_file=os.path.join(stage_name, poisson_output_numdofs_file_name)
        ),
        'pvd_file': os.path.abspath(
            os.path.join(stage_name, poisson_output_pvd_file_name)
        ),
...
def plot_over_line(
    poisson_output_pvd_file: str, poisson_output_vtu_file: str, source_directory: str
) -> str:
    ...
    _ = check_output(
        [
            'pvbatch',
            source_file_name,
            os.path.basename(poisson_output_pvd_file),
            pvbatch_output_file_name,
        ],
        prefix_name=stage_name,
        cwd=stage_name,
        universal_newlines=True,
...
def substitute_macros(
    pvbatch_output_file: str, ndofs: int, domain_size: float, source_directory: str
) -> str:
    stage_name = 'postprocessing'
    ...
    _copy_file_from_source(
        stage_name=stage_name,
        source_file_name=template_file_name,
        source_directory=source_directory,
    )
    ...
    _ = check_output(
        [
            'python',
            'prepare_paper_macros.py',
            '--macro-template-file',
            template_file_name,
            '--plot-data-path',
            os.path.basename(pvbatch_output_file),
            '--domain-size',
            str(domain_size),
            '--num-dofs',
            str(ndofs),
            '--output-macro-file',
            macros_output_file_name,
        ],
        prefix_name=stage_name,
        cwd=stage_name,
        universal_newlines=True,
...
def _poisson_collect_output(numdofs_file: str) -> int:
    with open(os.path.join(numdofs_file)) as f:
        return int(f.read())


def _copy_file(stage_name: str, source_file: str):
    input_file = os.path.join(
        os.path.abspath(stage_name), os.path.basename(source_file)
    )
...
def _copy_file_from_source(
    stage_name: str, source_file_name: str, source_directory: str
):
